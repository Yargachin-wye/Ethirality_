...
namespace GameAnalyticsSDK.Editor
{
    ...
    public class GA_SettingsInspector : UnityEditor.Editor
    {
        ...
        private GUIContent _publicKeyLabel =
            new GUIContent("Game Key", "Your GameAnalytics Game Key - copy/paste from the GA website.");
        private GUIContent _privateKeyLabel = new GUIContent("Secret Key",
            "Your GameAnalytics Secret Key - copy/paste from the GA website.");
        private GUIContent _emailLabel = new GUIContent("Email", "Your GameAnalytics user account email.");
        private GUIContent _passwordLabel = new GUIContent("Password",
            "Your GameAnalytics user account password. Must be at least 8 characters in length.");
        private GUIContent _organizationsLabel =
            new GUIContent("Org.", "Organizations tied to your GameAnalytics user account.");
        ...
        private GUIContent _build = new GUIContent("Build",
        ...
        private GUIContent _infoLogEditor = new GUIContent("Info Log Editor",
            "Show info messages from GA in the unity editor console when submitting data.");
        private GUIContent _infoLogBuild =
            new GUIContent("Info Log Build", "Show info messages from GA in builds (f.x. Xcode for iOS).");
        private GUIContent _verboseLogBuild = new GUIContent("Verbose Log Build",
        ...
        private GUIContent _useManualSessionHandling = new GUIContent("Use manual session handling",
        ...
        private GUIContent _enableSDKInitEvent = new GUIContent("Enable SDK Init Event (boot time on Android, iOS)",
        ...
        private GUIContent _enableHealthEvent = new GUIContent("Enable Session Performance Metrics",
            "Enables automatic performance data collection across the whole session. This includes sampling fps, memory consumption & cpu usage without any noticeable performance impact.");

        private GUIContent _enableMemoryTracking = new GUIContent("Health Memory Snapshots (Android, iOS)",
            "Performance & error events will take memory usage snapshots");
        private GUIContent _enableHardwareTracking = new GUIContent("Health Hardware Info (Android, iOS)",
            "Memory information collected (if available) and added as properties to health events. Determining total device memory, system memory usage and app memory usage");
        private GUIContent _enableFPSHistogram = new GUIContent("Submit Session FPS Histogram (Android, iOS)",
            "Enable FPS sampling across the entire session to ultimately send an FPS histogram at the end of the session. FPS insights can be reviewed in the GameAnalytics Health feature");
        private GUIContent _enableMemoryHistogram = new GUIContent("Submit Memory Usage Histogram (Android, iOS)",
            "Enable memory usage sampling across the entire session to ultimately send an memory histogram at the end of the session. Memory insights can be reviewed in the GameAnalytics Health feature");

#if UNITY_5_6_OR_NEWER
        private GUIContent _usePlayerSettingsBunldeVersionForBuild = new GUIContent(
            "Send Version* (Android, iOS) as build number",
 ...
 new GUIContent("Send Build number (iOS) and Version* (Android) as build number", "The SDK will automatically fetch the build number on iOS and the version* number on Android and send it as the GameAnalytics build number.");
        ...
        private GUIContent _account = new GUIContent("Account",
            "This tab allows you to login and automatically retrieve your Game Key and Secret Key.");
        private GUIContent _setup = new GUIContent("Setup",
            "This tab shows general options which are relevant for a wide variety of messages sent to GameAnalytics.");
        private GUIContent _advanced = new GUIContent("Advanced",
            "This tab shows advanced and misc. options for the GameAnalytics SDK.");
        private GUIContent _customDimensions01 =
            new GUIContent("Custom Dimensions 01", "List of custom dimensions 01.");
        private GUIContent _customDimensions02 =
            new GUIContent("Custom Dimensions 02", "List of custom dimensions 02.");
        private GUIContent _customDimensions03 =
            new GUIContent("Custom Dimensions 03", "List of custom dimensions 03.");
        ...
        private GUIContent _gaFpsAverage = new GUIContent("Submit Average FPS (Legacy)",
            "Submit the average frames per second. Warning: This FPS tracking approach will be replaced in a future update.");
        private GUIContent _gaFpsCritical = new GUIContent("Submit Critical FPS (Legacy)",
            "Submit a message whenever the frames per second falls below a certain threshold. The location of the Track Target will be used for critical FPS events. Warning: This FPS tracking approach will be replaced in a future update.");
        private GUIContent _gaFpsCriticalThreshold = new GUIContent("FPS <", "Frames per second threshold.");
        private GUIContent _gaSubmitErrors = new GUIContent("Submit Unity Errors Automatically",
        ...
        private GUIContent _gaNativeErrorReporting = new GUIContent("Submit Native Errors (Android, iOS) Automatically",
        ...
        private GUIContent _customDimensionsIconMsg =
            new GUIContent("Define your custom dimension values below. Values that are not defined will be ignored.");
        ...
        private GUIContent _healthEventIconMsg = new GUIContent(
            "Enable automatic tracking of events to discover and address issues related to how a game is technically running on devices/clients. Tracking options include errors, fps/memory usage histograms, app boot and hardware configuration");
        ...
        public override void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                if (ga.Organizations == null)
                {
                    ...
                    basicTabStyle = ga.CurrentInspectorState == GameAnalyticsSDK.Setup.Settings.InspectorStates.Basic
                        ? activeTabStyle
                        : inactiveTabStyle;
                }
                ...
                else if (ga.CurrentInspectorState == GameAnalyticsSDK.Setup.Settings.InspectorStates.Basic)
                {
                        ...
                        if (ga.PlatformFoldOut[i])
                        {
                                ...
                                i < ga.SelectedOrganization.Count)
                            {
                                ...
                                if (ga.SelectedOrganization[i] > 0)
                                {
                                    ...
                                    if (ga.SelectedStudio[i] > 0)
                                    {
                                        ...
                                        string[] gameNames = Studio.GetGameNames(ga.SelectedStudio[i] - 1,
                            ...
                            if (ga.SelectedPlatformGameID[i] >= 0)
                            {
                                ...
                                if (GUILayout.Button("Integration Status"))
                                {
                                    ...
                                    {
                                        Application.OpenURL(String.Format(_gaOverviewUrl,
                                            ga.SelectedPlatformGameID[i]));
                                    }
                                    else
                                    {
                                ...
                                if (GUILayout.Button("Game Settings"))
                                {
                                    if (string.IsNullOrEmpty(ga.TokenGA))
                                    {
                                        Application.OpenURL(String.Format(_gaSettingsUrl,
                                            ga.SelectedPlatformGameID[i]));
                                    }
                                    else
                                    {
                    ...
                    this.selectedPlatformIndex = EditorGUILayout.Popup("Platform to add", this.selectedPlatformIndex,
                    ...
                    if (GUILayout.Button("Add platform"))
                    {
                        ...
                        {
                            GameAnalytics.SettingsGA.AddPlatform((RuntimePlatform)System.Enum.Parse(
                                typeof(RuntimePlatform), this.availablePlatforms[this.selectedPlatformIndex]));
                        }
                    ...
                    if (_customDimensionsIconOpen)
                    {
                        ...
                        {
                            Application.OpenURL(
                                "https://docs.gameanalytics.com/integrations/sdk/unity/advanced-setup#custom-dimensions");
                        }
                    }
                    ...
                    if (_resourceTypesIconOpen)
                    {
                        ...
                        {
                            Application.OpenURL(
                                "https://docs.gameanalytics.com/integrations/sdk/unity/advanced-setup#resource-types");
                        }
                    }
                ...
                else if (ga.CurrentInspectorState == GameAnalyticsSDK.Setup.Settings.InspectorStates.Pref)
                {
                    ...
                    if (_advancedSettingsIconOpen)
                    {
                        ...
                        {
                            Application.OpenURL(
                                "https://docs.gameanalytics.com/integrations/sdk/unity/advanced-setup#advanced-settings");
                        }
                    }
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("", GUILayout.Width(-18));
                    ga.EnableSDKInitEvent =
                        EditorGUILayout.Toggle("", ga.EnableSDKInitEvent, GUILayout.Width(layoutWidth));
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("", GUILayout.Width(-18));
                    ga.EnableFPSHistogram =
                        EditorGUILayout.Toggle("", ga.EnableFPSHistogram, GUILayout.Width(layoutWidth));
                    GUILayout.Label(_enableFPSHistogram);
                    GUILayout.EndHorizontal();

                    GUILayout.BeginHorizontal();
                    GUILayout.Label("", GUILayout.Width(-18));
                    ga.EnableMemoryHistogram =
                        EditorGUILayout.Toggle("", ga.EnableMemoryHistogram, GUILayout.Width(layoutWidth));
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("", GUILayout.Width(-18));
                    ga.EnableHardwareTracking =
                        EditorGUILayout.Toggle("", ga.EnableHardwareTracking, GUILayout.Width(layoutWidth));
                    GUILayout.Label(_enableHardwareTracking);
                    GUILayout.EndHorizontal();

                    GUILayout.BeginHorizontal();
                    GUILayout.Label("", GUILayout.Width(-18));
                    ga.EnableMemoryTracking =
                        EditorGUILayout.Toggle("", ga.EnableMemoryTracking, GUILayout.Width(layoutWidth));
            ...
            try
            {
...
#endif
                {
                    ...
                    else if (returnParam != null)
                    {
                        ...
                        Dictionary<string, GameAnalyticsSDK.Setup.Organization> organizationMap =
                            new Dictionary<string, GameAnalyticsSDK.Setup.Organization>();
                        List<GameAnalyticsSDK.Setup.Organization> returnOrganizations =
                            new List<GameAnalyticsSDK.Setup.Organization>();
                        foreach (KeyValuePair<string, object> pair in orgs)
                        {
                            ...
                            GameAnalyticsSDK.Setup.Organization o =
                                new GameAnalyticsSDK.Setup.Organization(organization["name"].ToString(),
                        ...
                        for (int s = 0; s < studioList.Count; s++)
                        {
                                ...
                                (!studio.ContainsKey("archived") || !((bool)studio["archived"])))
                            {
                                ...
                                for (int g = 0; g < gamesList.Count; g++)
                                {
                                    ...
                                    {
                                        returnGames.Add(new GameAnalyticsSDK.Setup.Game(game["name"].ToString(),
                                            int.Parse(game["id"].ToString()), game["key"].ToString(),
                                            game["secret"].ToString()));
                                    }
                                }

                                GameAnalyticsSDK.Setup.Studio st = new GameAnalyticsSDK.Setup.Studio(
                                    studio["name"].ToString(), studio["id"].ToString(), studio["org_id"].ToString(),
                                    returnGames);
        ...
        public static void CheckForUpdates()
        {
            ...
            UnityWebRequest www =
                UnityWebRequest.Get("https://s3.amazonaws.com/public.gameanalytics.com/sdk_status/current.json");
...
#if UNITY_2017_1_OR_NEWER
            UnityWebRequest www =
                UnityWebRequest.Get("https://s3.amazonaws.com/public.gameanalytics.com/sdk_status/change_logs.json");
#else
        ...
        private static IEnumerator CheckForUpdatesCoroutine(UnityWebRequest www)
        {
            ...
            try
            {
...
#endif
                {
                    ...
                    text = www.text;
#endif
                    IDictionary<string, object> returnParam =
                        GA_MiniJSON.Deserialize(text) as IDictionary<string, object>;
                    if (returnParam.ContainsKey("unity"))
                    {
                        ...
                        {
                            string newVersion = (returnParam["unity"] as IDictionary<string, object>)["version"]
                                .ToString();
        ...
        private static void OpenUpdateWindow()
        {
...
#endif
            {
                ...
                GA_UpdateWindow updateWindow =
                    (GA_UpdateWindow)EditorWindow.GetWindow(typeof(GA_UpdateWindow), utility: true);
